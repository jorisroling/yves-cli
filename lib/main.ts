...
const sift = _sift as unknown as (
...
export class Main {
  ...
  parse(json: string): Record<string, unknown> | undefined {
    if (json.indexOf("{") >= 0) {
      return JSON.parse(
        json.substring(0, 5) === ")]}'," ? json.substring(5) : json,
      );
    }
  }
  ...
  doProcess(object: unknown): void {
    ...
    if (this.options.query) {
      if (Array.isArray(object)) {
        ...
        console.error(
          `Data root is not of type array (but of type ${typeof object}), so query is not possible.` +
            (this.options.root ? "" : " Maybe --root can help?"),
        );
        return;
      }
    }
    ...
    if (this.options.fields) {
      if (Array.isArray(object)) {
        ...
        object = object.map((item: Record<string, unknown>) =>
          pick(item, fields)
        );
      } else {
        console.error(
          `Data root is not of type array (but of type ${typeof object}), so fields is not possible.` +
            (this.options.root ? "" : " Maybe --root can help?"),
        );
        return;
      }
  ...
  command(args: string[]): void {
    ...
    if (args.length > 0) {
      ...
      process.stdin.on("end", () => setTimeout(() => Deno.exit(0), 0));
