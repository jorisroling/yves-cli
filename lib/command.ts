...
export default async function () {
  const isInfoFlag = Deno.args.includes("--version") ||
    Deno.args.includes("-V") || Deno.args.includes("--help") ||
    ...
    .action((opts, ...files: string[]) => {
      ...
      try {
        ...
        if (err instanceof CustomError) {
          console.error(
            opts.color ? red(`ERROR: ${err.message}`) : `ERROR: ${err.message}`,
          );
          Deno.exit(1);
        } else if (err instanceof SyntaxError) {
          console.error(
            opts.color ? red("ERROR: Invalid JSON") : "ERROR: Invalid JSON",
          );
          throw err;
        } else {
          throw err;
